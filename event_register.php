<?php
	...
	echo'<section id="form-design">
		<div class="col-sm-offset-3 col-sm-6">
			<h3 class="text-center text-success">';echo($row['eventname']."<br>".' Event Time: '.$row['start_date']."<br>".'Ticket is Available'/*.$ticketNumber*/).'</h3><hr>';
			}
	...
	$sql="CREATE TABLE IF NOT EXISTS event_register(
		...
		if(!mysqli_query($conn,$sql)){
			...
			//echo"Table Created";
		$qrNumber=$id.rand(1,9).date('dHis');//echo$qrNumber;
		if(isset($_POST['submit'])){
			...
			$sql="SELECT * FROM event_register WHERE email='$email' AND eventID='$id'";
				$result=mysqli_query($conn,$sql);
				if(mysqli_num_rows($result)){
					 echo'<div class="col-sm-offset-3 col-sm-6 alert alert-warning">
  <strong>You Already Registered For This Event!</strong>
</div>';
			}else{
				$sql="INSERT INTO event_register(fullName,phone,email,district,status,qrnumber,eventId)
			VALUE('$fullname','$phone','$email','$district','$status','$qrNumber','$id')";
			if(!mysqli_query($conn,$sql)){
				echo('<div class="col-sm-offset-3 col-sm-6 alert alert-warning">There was an problem. Try again'.mysqli_error($conn)).'</div>';
			}
			else
